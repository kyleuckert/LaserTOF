...
class labTOF(Frame):
	...
	def __init__(self, parent):
		...
		self.toolbar.grid(row=0, column=0, sticky=N+S+W+E)
	...
	def initUI(self):
		...
		#self.pack(fill=BOTH, expand=1)
		#self.grid()
		self.grid(row=0, column=0, sticky=W+E+N+S, padx=5, pady=5)
		...
		quitButton.grid(row=2, column=0, padx=5, pady=5, sticky=E)
		...
		self.parentButton.grid(row=2, column=0, padx=5, pady=5, sticky=W)
		...
		self.calibrateButton.grid(row=2, column=0, padx=5, pady=5)
	...
	def generate_figure(self, data, label, xaxis_title):
		...
		self.canvas.get_tk_widget().grid(row=1, column=0, sticky=N+S+E+W)
